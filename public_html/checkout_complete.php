	...
	if (!$_SESSION['payment']) { 
		...
		?>
			...
			<h2>Your order is complete. A summary is below:</h2>
				...
				$qry3 = "SELECT item, image_id, quantity, price FROM JJ_order_details WHERE order_id = :orderID";
				...
				//echo $result['item'];
				for($i = 1; $i < $itemNum; $i++) { //retrieve the cart variables from the session
					$result = $stmt3->fetch();
					echo "Item Number: ".$result['item']."<br>";
					echo "Image ID: ".$result['image_id']."<br>";
					echo "Quantity: ".$result['quantity']."<br>";
					echo "Price: ".$result['price']."<br><br>";
					
					/*$imageID = $img;
					$qty = $item['quantity'];
					$price = $item['price'];
					$stmt3->bindValue(':item', $itemNum);
					$stmt3->bindValue(':orderID', $orderID);
					$stmt3->bindValue(':imageID', $imageID);
					$stmt3->bindValue(':qty', $qty);
