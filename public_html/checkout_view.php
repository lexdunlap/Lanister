...
<main>
    <?php if (empty($_SESSION['cart']) || !isset($_SESSION['cart'])) { 
		...
		//header('Location: http://'. $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/address.php');
		echo "<h3><a href='address.php'>Click to enter your shipping information</a><h3>";
	} else {
		//check for existing address for this  registered user:
		$email = $_SESSION['email'];
		$query = 'SELECT * FROM 350addresses WHERE customerEmail = :email';
